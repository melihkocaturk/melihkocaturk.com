---
layout: page
title: İletişim
permalink: /iletisim
---

Gayrimenkul ile ilgili her türlü soru, öneri ya da iş birliği talebiniz için benimle iletişime geçebilirsiniz.
Evinizi satmak, yeni bir yaşam alanı bulmak ya da yatırım fırsatlarını değerlendirmek istiyorsanız, doğru yerdesiniz.

Size en iyi şekilde yardımcı olabilmem için aşağıdaki iletişim kanallarını kullanarak bana ulaşabilirsiniz.

## İletişim Bilgileri

- **E-mail**: <a href="mailto:{{ site.author.email }}">{{ site.author.email }}</a>
- **Konum**: İstanbul, Göktürk

## Çalışma Saatleri

- **Hafta içi**: 09:00 – 18:00
- **Cumartesi**: 09:00 – 12:00
- **Pazar**: Randevu ile